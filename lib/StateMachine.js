...
function StateMachine (params) {
  ...
  /**
   * Allows asynchonous waiting for a state to enter
   *
  ...
  async function waitUntilState (state, timeout = 0) {
    ...
    if (timeout > 0) await stateUpdate.timeout(timeout)
  ...
  function _init () {
    ...
    for (let row of _transitions) {
      ...
      if (_allowedStates.has(row.ev)) {
        _allowedStates[row.ev].push(row.from)
      } else {
        _allowedStates[row.ev] = [row.from]
        _events[onEv] = () => {}
      }
      ...
      _events[row.ev] = async (...args) => {
        ...
        if (_allowedStates[row.ev].includes(_state)) {
